...
label-crds: $(BUILD_DIRS)
	@for f in .crds/*.yaml; do \
		echo "applying app.kubernetes.io/name=cluster-api label to $$f"; \
		kubectl label --overwrite -f $$f --local=true -o yaml app.kubernetes.io/name=cluster-api > bin/crd.yaml; \
		mv bin/crd.yaml $$f; \
	done
...
gen-values-schema: $(BUILD_DIRS)
	@for dir in charts/*/; do \
		dir=$${dir%*/}; \
		dir=$${dir##*/}; \
		crd_file=.crds/installer.cluster-api.dev_$$(echo $$dir | tr -d '-')s.yaml; \
		if [ ! -f $${crd_file} ]; then \
			continue; \
		fi; \
